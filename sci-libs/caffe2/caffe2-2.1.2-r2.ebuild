...
SLOT="0"
KEYWORDS="~amd64"
IUSE="cuda distributed fbgemm ffmpeg gloo mkl mpi nnpack +numpy onednn opencl opencv openmp qnnpack rocm tensorpipe xnnpack"
RESTRICT="test"
REQUIRED_USE="
	${PYTHON_REQUIRED_USE}
	ffmpeg? ( opencv )
...
PATCHES=(
	...
	"${FILESDIR}"/${PN}-2.1.2-rocm-gentoo.patch
)
...
src_configure() {
	...
	if use cuda; then
		...
		mycmakeargs+=(-DUSE_ROCM=OFF -DUSE_CUDA=OFF)
	fi
